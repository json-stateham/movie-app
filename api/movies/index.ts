...
export const getMovieDetails = (
  ...
  reqUrl.addPath('movie', movieId);
  reqUrl.addParams({ ...API_CONFIG.APPEND_MOVIE_DETAILS, ...params });
  return jsonFetch(reqUrl.href);
};

export const getMoviesList = (movieCategory: TMovieCategory, page: number) => {
  const reqUrl = BASE_MOVIE_URL.clone();
  reqUrl.addPath('movie', movieCategory);
  reqUrl.addParams({ page: String(page) });
  return jsonFetch(reqUrl.href);
};

export const getGenres = () => {
  const reqUrl = BASE_MOVIE_URL.clone();
  reqUrl.addPath('genre', 'movie', 'list');
  return jsonFetch(reqUrl.href);
}
